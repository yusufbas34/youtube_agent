...
def create_history_video(content: dict, output_path: str = None) -> str:
    ...
    # Müzik: GitHub'daki telif-free tarih müziği
    try:
        from music_manager import get_tarih_music, find_music_by_keywords, get_fallback_music
        music_path = get_tarih_music()
        if not music_path:
            music_path = find_music_by_keywords(["tarih","ozlu","epik","huzur","yasam","felsefe"])
    ...
    # ── Bellek temizleme (RAM leak önleme) ──
    try:
        final.close()
        content_clip.close()
        intro_clip.close()
        ...
        for a in seg_audio_clips:
            try: a.close()
            except: pass
        try: intro_audio.close()
        except: pass
    except: pass
